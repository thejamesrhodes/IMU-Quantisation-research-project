...
$\rhod$   & $\sigma/\Dlsb$, dither ratio & --- & independent variable \\
$\phase$  & sub-code bias phase & $\Dlsb$ & \textbf{edge}-referenced \\
$\etaq$   & normalised quantisation power $/(\Dlsb^2/12)$ & --- & $+1$ is the PQN limit \\
$x$       & 20-bit reference stream & $\Dlsb$ & $\mathrm{gyro}_{20}/16$ \\
$Q$       & 16-bit register stream & $\Dlsb$ & \textbf{not} Kalman $Q$ or RRW density \\
$v$       & quantiser input signal & $\Dlsb$ & before quantisation \\
$\mu$     & sample mean & $\Dlsb$ & mean of $x$ \\
$\sigma$  & sample standard deviation & $\Dlsb$ & from reference stream only \\
$\operatorname{Var}(\cdot)$ & variance & $\Dlsb^2$ & sample variance operator \\
\bottomrule
\end{tabular}
\end{table}
