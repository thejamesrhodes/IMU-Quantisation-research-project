...
#define SCAN_PERIOD_MS   1000U     /* re-scan cadence; lets you probe live      */
...
extern volatile uint32_t g_cdc_rx_len;
extern uint8_t           g_cdc_rx_buf[64];
static volatile int g_burst_active = 0;
...
int main(void)
{
  ...
  print_banner();
  scan_all_buses();                            /* one immediate scan at boot */

  char line[80];
  int n;
  for (int i = 0; i < 4; i++) {
    n = snprintf(line, sizeof line,
      "bus%d: ICM=0x%02X ISM=0x%02X BMI=0x%02X present=%d\r\n",
      i+1, spi_read8(&g_buses[i],0x75), spi_read8(&g_buses[i],0x0F),
      bmi323_chipid(&g_buses[i]), bus_present(&g_buses[i]));
  ...
  HAL_Delay(2000);        /* let the host enumerate */

  uint32_t last_scan = HAL_GetTick();   /* seed once, before the loop starts */
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  if (HAL_GetTick() - last_scan >= SCAN_PERIOD_MS) {
	    last_scan = HAL_GetTick();
	    scan_all_buses();
	    for (int i = 0; i < 4; i++) sensor_mvp_log(&g_buses[i], i);
	  }

	  static uint32_t last_cdc = 0;
      if (HAL_GetTick() - last_cdc >= 1000U) {
        last_cdc = HAL_GetTick();
        static uint32_t seq = 0;
        static uint8_t last_state = 0xFF;
        if (hUsbDeviceHS.dev_state != last_state) {
          last_state = hUsbDeviceHS.dev_state;
          uart_log("USB dev_state -> %d\r\n", (int)last_state);
        }
        cdc_log("cdc seq=%lu t=%lu\r\n",
                (unsigned long)seq++, (unsigned long)HAL_GetTick());
      }

      if (g_cdc_rx_len) {
      	        uint32_t n = g_cdc_rx_len;
      	        g_cdc_rx_len = 0;

      	        if (n >= 1 && g_cdc_rx_buf[0] == 'b') {
      	          cdc_burst(200);
      	        }
      	        else if (n >= 20 && g_cdc_rx_buf[0] == 'T') {
      	          RTC_TimeTypeDef t = {0};
      	          RTC_DateTypeDef d = {0};
      	          const char *s = (const char *)g_cdc_rx_buf;
      	          d.Year    = (s[3]-'0')*10  + (s[4]-'0');
      	          d.Month   = (s[6]-'0')*10  + (s[7]-'0');
      	          d.Date    = (s[9]-'0')*10  + (s[10]-'0');
      	          t.Hours   = (s[12]-'0')*10 + (s[13]-'0');
      	          t.Minutes = (s[15]-'0')*10 + (s[16]-'0');
      	          t.Seconds = (s[18]-'0')*10 + (s[19]-'0');
      	          ...
      	          cdc_write(g_cdc_rx_buf, (uint16_t)n);
      	          uart_log("cdc rx %lu B\r\n", (unsigned long)n);
      	        }
      	      }

      static uint32_t t_rate = 0;
      	  static uint32_t c0[4]  = {0};
      	  if (HAL_GetTick() - t_rate >= 10000U) {
      	    uint32_t dt = HAL_GetTick() - t_rate;
      	    t_rate = HAL_GetTick();
      	    for (int i = 0; i < 4; i++) {
      	      uint32_t c  = g_drdy_count[i];
      	      uint32_t dn = c - c0[i];
      	      c0[i] = c;
      	      uart_log("bus%d drdy: %lu edges in %lu ms -> %lu.%02lu Hz\r\n",
      	               i + 1, (unsigned long)dn, (unsigned long)dt,
      	               (unsigned long)(dn * 1000UL / dt),
      	               (unsigned long)(dn * 100000UL / dt % 100UL));
      	    }
      	  }
...
/* Blocking printf-over-UART. Fine at 115200 for diagnostic-rate output;
   don't use this on a hot data path once you move to real IMU capture. */
static void uart_log(const char *fmt, ...)
{
  if (huart1.gState == HAL_UART_STATE_RESET) return;   /* UART not up yet */
  char buf[160];
  va_list ap;
  va_start(ap, fmt);
  ...
  va_end(ap);
  if (n < 0) return;
  if (n >= (int)sizeof buf) n = (int)sizeof buf - 1;   /* was `> sizeof buf` */
...
static void print_banner(void)
{
  ...
  uart_log("Scanning 4 SPI buses every %lu ms. Ctrl+wiggle connectors live.\r\n",
            (unsigned long)SCAN_PERIOD_MS);
  uart_log("=============================================================\r\n");
}
...
static void usb_report(void)
{
  ...
  uart_log("USB dev_state=%d (3=CONFIGURED)\r\n", (int)hUsbDeviceHS.dev_state);
  if (hUsbDeviceHS.dev_state != USBD_STATE_CONFIGURED)
    uart_log("  -> not enumerated: cable, VBUS, or PHY clock\r\n");
}
